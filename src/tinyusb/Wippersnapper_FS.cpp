...
void Wippersnapper_FS::parseSecrets() {
  ...
  if (err) {
    Serial.println("ERROR: deserializeJson() failed with code ");
  ...
  if (network_type_wifi_ssid != nullptr) {
    network_ssid = network_type_wifi_ssid;
  }

  // error check against default values [ArduinoJSON, 3.3.3]
  if (network_ssid == nullptr) {
    Serial.println("ERROR: invalid network_ssid value in secrets.json!");
  ...
  if (strcmp(network_ssid, "YOUR_WIFI_SSID_HERE") == 0) {
    writeToBootOut(
  ...
  if (network_type_wifi_password != nullptr) {
    network_pass = network_type_wifi_password;
  }
...
void Wippersnapper_FS::writeToBootOut(PGM_P str) {
  ...
  if (bootFile) {
    ...
    Serial.println("ERROR: Unable to open wipper_boot_out.txt for logging!");
  }
}
...
displayConfig Wippersnapper_FS::parseDisplayConfig() {
  ...
  if (!wipperFatFs.exists("/display_config.json")) {
    Serial.print("Could not find display_config.json, generating...");
    createDisplayConfig();
    Serial.println("done!");
  }
